		...
		unsigned int flags, struct iomap *iomap)
{
	...
	/*
	 * The iomap core validates that inline_data + length doesn't
	 * cross a page boundary (iomap_inline_data_valid).  kmemdup()
	 * returns slab memory with arbitrary page offsets, which can
	 * violate that constraint.  Use a page-aligned allocation to
	 * guarantee offset_in_page(inline_data) == 0.
	 */
	iomap->inline_data = (void *)__get_free_page(GFP_KERNEL);
	if (!iomap->inline_data) {
		err = -ENOMEM;
		goto out;
	}
	memcpy(iomap->inline_data, kattr, attr_len);
		...
		ssize_t written, unsigned int flags, struct iomap *iomap)
{
	if (iomap->type == IOMAP_INLINE)
		free_page((unsigned long)iomap->inline_data);
		...
		struct iomap *iomap)
{
	...
	/* See comment in ntfs_read_iomap_begin_resident(). */
	iomap->inline_data = (void *)__get_free_page(GFP_KERNEL);
	if (!iomap->inline_data) {
		err = -ENOMEM;
		goto out;
